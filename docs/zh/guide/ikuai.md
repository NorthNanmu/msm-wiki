...
- **故障时**: 修改 DHCP DNS 为 `223.5.5.5`
- **恢复时**: 修改 DHCP DNS 为 `192.168.1.2`

::: warning 注意
爱快的健康检查功能可能因版本而异，部分版本可能不支持自动修改 DHCP DNS。如果不支持，可以使用脚本实现。
:::
...
进入 **网络设置 > DHCP 设置 > DHCP 服务端**，确认 DHCP 下发 DNS 已设置为 `192.168.1.2`。
