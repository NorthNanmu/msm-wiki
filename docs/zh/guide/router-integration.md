...
将主路由 DHCP 下发的 DNS 服务器设置为 MSM 主机的 IP 地址。

**示例**：
- MSM 主机 IP: `192.168.1.2`
- 主路由 DHCP DNS 下发: `192.168.1.2`
...
MSM 的 MosDNS 负责 DNS 分流，只有将主路由 DHCP 下发的 DNS 指向 MSM，设备的 DNS 请求才会被 MosDNS 处理，才能实现国内外域名的智能分流。
